...
public class Average180Line {
	private Line2D.Double average;
	private final int candleWidth = 3;
	
	public Average180Line(Day day, Day prevDay, int index, double scale, double chartBaseY, double segmentLow) {
		average = new Line2D.Double();
		average.x1 = index * candleWidth;
		average.y1 = chartBaseY - prevDay.getAvg180() * scale + segmentLow * scale;
		average.x2 = (index+1) * candleWidth;
		average.y2 = chartBaseY - day.getAvg180() * scale + segmentLow * scale;
	}
